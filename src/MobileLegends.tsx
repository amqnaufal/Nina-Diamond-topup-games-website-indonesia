...
import { ChevronRight, Search, LogIn, UserPlus, Star, Flame, ChevronLeft, ChevronDown, LayoutGrid, Settings, LogOut } from 'lucide-react';
          ...
          <div className="order-2 flex w-full flex-1 gap-2 md:ml-4 items-center md:w-auto">
            ...
            <div className="relative profile-dropdown-container">
              <button
                ...
                className="flex h-8 w-8 md:h-10 md:w-10 shrink-0 overflow-hidden rounded-full border-2 border-white/15 transition hover:border-[#f27221] bg-[#333333] drop-shadow-sm cursor-pointer justify-center items-center"
              >
                {user ? (
                  user.photoURL ? (
                    <img 
                    ...
                    <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-[#f27221]/30 to-red-600/30 hover:from-[#f27221]/40 hover:to-red-600/40 text-[10px] md:text-xs font-bold text-white uppercase tracking-wider">
              ...
              {user && isProfileDropdownOpen && (
                ...
                >
                  ...
                  <div className="mb-4 flex items-center justify-between rounded-lg bg-black/20 p-2.5">
                    ...
                    {/* Lumos Coins term */}
                    <span className="text-xs font-bold text-[#f27221]">
                      Nina Coins
                    </span>
                  </div>
          ...
          <div className="flex shrink-0 items-center justify-end gap-4 md:gap-6 text-xs md:text-sm ml-4 py-3">
            {user && (
              <div className="flex items-center gap-4">
                <span className="font-semibold text-[#f27221] hidden md:block">Hi, {userProfile?.fullName || user.displayName || user.email?.split('@')[0] || 'User'}</span>
              </div>
            )}
          </div>
